...
int kzCheckhash(const char *filnam, char **zipnam, int *fileoffs, int *fileleng, char *iscomp)
{
	int i;
...
#elif defined(_WIN32)
			hfind = FindFirstFileA(wildst, &findata);
			if( hfind == INVALID_HANDLE_VALUE )
			{
				if( !kzhashbuf ) return(0); srchstat = kzlastfnam; continue;
			}
...
int kzRead(void *buffer, int leng)
{
	...
	LogPrint("kzfs.pos=" + std::to_string(kzfs.pos));
	где-то тут
	...
	if( kzfs.comptyp == 0 )
	{
		...
		i = (int)fread(buffer, i, 1, kzfs.fil);
		//fread(buffer, i, 1, kzfs.fil); // TODO: тут ошибка? выше правильный вариант
	...
	else if( kzfs.comptyp == 8 )
	{
		...
		if( kzfs.pos < gslidew - 32768 ) // Must go back to start :(
		{
			...
			// fread(&olinbuf[0], kzfs.comptell, 1, kzfs.fil); // OLD
